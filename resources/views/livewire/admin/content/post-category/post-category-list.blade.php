    ...
    <div class="card-body">
        <div class="table overflow-auto" tabindex="8">
            <div class="d-flex justify-content-end mb-2">
                <a href="{{ route('admin.content.post_category.create') }}" class="btn btn-outline-success">افزودن</a>
            </div>
                    ...
                    <tr>
                        ...
                        <td class="text-center align-middle">
                            <figure class="avatar avatar">
                                @if($category->image)
                                <img src="{{ asset($category->image['indexArray'][$category->image['currentImage']] ) }}" class="rounded-circle" alt="image">
                                @endif
                            </figure>
                        </td>                        <td class="text-center align-middle"><span wire:click="changeStatus({{ $category->id }})" class="cursor-pointer badge @if($category->status) badge-success @else badge-danger @endif">{{ $category->status ? 'فعال' : 'غیرفعال' }}</span></td>
                        <td class="text-center align-middle">
                            <a class="btn btn-outline-info" href="{{ route('admin.content.post_category.edit', $category->id) }}">
                                ویرایش
                            </a>
                        </td>
                        <td class="text-center align-middle">
